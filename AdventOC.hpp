...
namespace advoc{
    ...
    uint readInput(std::string filepath, char* w_start, uint bufsize){
        std::fstream reader{filepath};
        if (!reader.is_open()){
            std::cerr << "Shit broke!\n";
        }
        uint read_amount = reader.readsome(w_start, bufsize);
        reader.close();
        return read_amount;
    }


    std::vector<std::string> stringSplit(std::string str_arg, char split_by, uint8_t skip_all=1){
        /*
        ...
        uint8_t skip_all - Skip all consecutive split chars as well as spaces, 1 (true) by default, if 0 (false) then only first split char is used
        RETURNS:
